...
export default async function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-[#333333] font-sans pb-20">
      {/* Header */}
      <header className="bg-[#5C2D0C] text-white py-5 px-4 md:px-8 flex flex-col md:flex-row items-center justify-between shadow-md sticky top-0 z-10 gap-3 md:gap-0">
        <h1 className="text-lg md:text-2xl font-bold flex items-center gap-2">
          🏥 병원·보건의료 종합 모닝 브리핑
        </h1>
        <div className="flex items-center gap-2 bg-white/20 px-4 py-1.5 rounded-full text-xs md:text-sm">
          <Calendar className="w-4 h-4" />
          <span>{today} 기준 최신 게시물</span>
        </div>
      </header>
